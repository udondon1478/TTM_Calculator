...
function App() {
  ...
  const checkTtmStatus = async () => {
    try {
      ...
      // Safely log error without using console.error directly with the error object
      console.log('Failed to check TTM status:', error?.message || 'Unknown error');
  ...
  const refreshTtmData = async () => {
    try {
      ...
      // Safely log error without using console.error directly with the error object
      console.log('Failed to refresh TTM data:', error?.message || 'Unknown error');
        ...
        {results && !isLoading && (
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6">
            ...
            <div className="mt-8 p-4 bg-gray-100 dark:bg-gray-700 rounded-md">
              <h3 className="text-lg font-medium text-gray-800 dark:text-white mb-2">年間集計</h3>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="bg-white dark:bg-gray-800 p-4 rounded-md shadow">
                  <p className="text-sm text-gray-600 dark:text-gray-400">総取引数</p>
                  <p className="text-2xl font-bold">{results?.summary?.totalTransactions || 0}</p>
                </div>
                <div className="bg-white dark:bg-gray-800 p-4 rounded-md shadow">
                  <p className="text-sm text-gray-600 dark:text-gray-400">USD合計</p>
                  <p className="text-2xl font-bold">${results?.summary?.totalUsd?.toFixed(2) || 0}</p>
                </div>
                <div className="bg-white dark:bg-gray-800 p-4 rounded-md shadow">
                  <p className="text-sm text-gray-600 dark:text-gray-400">JPY合計</p>
                  <p className="text-2xl font-bold">¥{results?.summary?.totalJpy?.toLocaleString() || 0}</p>
                </div>
              </div>
            </div>
          </div>
